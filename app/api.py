...
@app.get("/health")
async def health(settings: Settings = Depends(get_settings)) -> dict[str, str | int]:
    return {
        "status": "ok",
...
async def analyze(
    ...
    resume_files: Annotated[list[UploadFile] | None, File()] = None,
    blind_mode: Annotated[bool, Form()] = True,
    settings: Settings = Depends(get_settings),
    llm: NvidiaLLMProvider = Depends(provider),
) -> AnalyzeResponse:
    ...
    if count == 0 or count > settings.max_resumes_per_request:
        raise HTTPException(
            422, f"Provide between 1 and {settings.max_resumes_per_request} resumes."
        )
    try:
        ...
        job = (
            jd_text.strip()
            if jd_text
            else parse_document(jd_file.filename or "job", await jd_file.read(), limit)
        )
        resumes = []
        if resume_text and resume_text.strip():
            ...
            resumes.append(
                (
                    "Pasted resume",
                    text,
                    extract_github_repositories("resume.txt", text.encode(), text),
                )
            )
        for file in files:
            ...
            resumes.append(
                (filename, text, extract_github_repositories(filename, content, text))
            )
        repository = ScreeningRepository(settings.database_url)
        github = GitHubVerifier(settings, llm)
        return await ScreeningService(
            llm, repository, settings.max_concurrent_evaluations, github
        ).analyze(job, resumes, blind_mode)
...
async def verify_github(
    ...
    try:
        return await GitHubVerifier(settings, llm).verify(
            request.repository_url, request.claimed_description
        )
    except ValueError as exc:
        raise HTTPException(422, str(exc)) from exc
    except (httpx.HTTPError, LLMError) as exc:
        raise HTTPException(
            502, "GitHub verification is temporarily unavailable."
        ) from exc
...
async def list_jobs(
    ...
    return JobDashboardResponse(
        jobs=await ScreeningRepository(settings.database_url).list_jobs()
    )
...
async def get_candidate_report(
    ...
    result = await ScreeningRepository(settings.database_url).get_candidate(
        job_id, candidate_id
    )
    if result is None:
